...
export class APISlotProvidersService {
      ...
      providers.map(async (provider) => {
        try {
          ...
          const convertedGames = games.map((game) =>
            this.convertAPIGameToSocialCasino(game, provider),
          );
  ...
  static async fetchGamesFromProvider(
    providerId: string,
  ): Promise<APIProviderGame[]> {
  ...
  private static async simulateFetchFromAPI(
    provider: APIProvider,
  ): Promise<APIProviderGame[]> {
    ...
    const gameNames = [
      ...
      "Neon City Nights",
      "Space Odyssey 2077",
      "Cyber Punk Revolution",
      "Ocean's Deep Mystery",
      "Jungle Safari Wild",
      "Arctic Adventure",
      "Diamond Luxury Club",
      "Ruby Fortune Palace",
      "Emerald Kingdom",
      "Crystal Cave Explorer",
      "Fire Dragon's Lair",
      "Ice Queen's Castle",

      // Fantasy themed games
      "Wizard's Magic Realm",
      "Fairy Tale Dreams",
      "Enchanted Forest",
      "Mermaid's Underwater World",
      "Phoenix Rising Fire",
      "Unicorn's Rainbow",
      "Castle of Shadows",
      "Angel's Divine Light",
      "Demon's Dark Portal",
      ...
      "Classic Fruit Bonanza",
      "Lucky 7s Jackpot",
      "Triple Cherry Blast",
      "Diamond Triple Slots",
      "Gold Bar Extravaganza",
      "Royal Crown Jewels",
      "Lucky Clover Fields",
      "Rainbow Fruit Mix",
      "Mega Fruit Explosion",
  ...
  private static generateGameThumbnail(
    gameName: string,
    theme: string,
  ): string {
    const colors = {
  ...
  private static async simulateSpinAPI(
    ...
    return {
      ...
      result: {
        ...
        bonusTriggered,
        freeSpinsAwarded: bonusTriggered
