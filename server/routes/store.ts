...
import { GoldCoinPackage, PurchaseTransaction, StoreSettings, StorePromotion, AdminLog, UserPurchaseHistory, RefundRequest } from "@shared/storeTypes";
...
const initializePackages = () => {
  const defaultPackages: GoldCoinPackage[] = [
    ...
    },
    {
      ...
      popular: true,
      bestValue: false,
      features: ['25,000 Gold Coins', '15 Bonus Sweeps Coins', '20% Extra Bonus', 'Priority Support'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      popular: false,
      bestValue: true,
      features: ['50,000 Gold Coins', '35 Bonus Sweeps Coins', '40% Extra Bonus', 'VIP Support', 'Exclusive Games'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      popular: false,
      bestValue: false,
      features: ['100,000 Gold Coins', '75 Bonus Sweeps Coins', '50% Extra Bonus', 'VIP Treatment', 'All Game Access', 'Monthly Rewards'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      popular: false,
      bestValue: false,
      features: ['250,000 Gold Coins', '200 Bonus Sweeps Coins', '75% Extra Bonus', 'Platinum VIP', 'Personal Account Manager', 'Exclusive Tournaments'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      popular: false,
      bestValue: false,
      features: ['5,000 Gold Coins', '2 Bonus Sweeps Coins', 'Daily Login Bonus'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      popular: false,
      bestValue: false,
      features: ['75,000 Gold Coins', '50 Bonus Sweeps Coins', 'Weekend Multiplier', 'Bonus Games Access'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    },
    {
      ...
      popular: false,
      bestValue: false,
      features: ['500,000 Gold Coins', '500 Bonus Sweeps Coins', '100% Extra Bonus', 'Diamond VIP', 'Concierge Service', 'Private Gaming Room'],
      isActive: true,
      createdAt: new Date(),
      updatedAt: new Date()
    }
...
export const handlePurchasePackage: RequestHandler = (req, res) => {
  const { userId, username, packageId, paymentMethod, paymentReference } = req.body;
...
export const handleGetUserPurchases: RequestHandler = (req, res) => {
  ...
  const history: UserPurchaseHistory = {
    userId,
    transactions: userTransactions.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()),
    totalSpent: userTransactions.filter(t => t.status === 'completed').reduce((sum, t) => sum + t.amountPaid, 0),
    totalGoldCoins: userTransactions.filter(t => t.status === 'completed').reduce((sum, t) => sum + t.goldCoinsAwarded, 0),
    totalSweepsCoins: userTransactions.filter(t => t.status === 'completed').reduce((sum, t) => sum + t.sweepsCoinsBonus, 0),
    firstPurchase: userTransactions.length > 0 ? new Date(Math.min(...userTransactions.map(t => t.createdAt.getTime()))) : new Date(),
    lastPurchase: userTransactions.length > 0 ? new Date(Math.max(...userTransactions.map(t => t.createdAt.getTime()))) : new Date(),
    vipStatus: 'none'
  };
...
export const handleGetPaymentStats: RequestHandler = (req, res) => {
  ...
  const todayTransactions = completedTransactions.filter(t => t.completedAt && t.completedAt >= startOfDay);
  
  const stats = {
    totalRevenue: completedTransactions.reduce((sum, t) => sum + t.amountPaid, 0),
    ...
    averageTransactionValue: completedTransactions.length > 0 ? 
      completedTransactions.reduce((sum, t) => sum + t.amountPaid, 0) / completedTransactions.length : 0,
...
const getPopularPackages = (transactions: PurchaseTransaction[]) => {
  const packageCounts: Record<string, { count: number; revenue: number; name: string }> = {};
  
  transactions.forEach(t => {
    if (!packageCounts[t.packageId]) {
      packageCounts[t.packageId] = { count: 0, revenue: 0, name: t.packageName };
    }
...
const getRevenueByPaymentMethod = (transactions: PurchaseTransaction[]) => {
  ...
  transactions.forEach(t => {
    methodRevenue[t.paymentMethod] = (methodRevenue[t.paymentMethod] || 0) + t.amountPaid;
  });
