...
export const handlePlaceBet: RequestHandler = (req, res) => {
  ...
  const potentialWin =
    Math.round(stake * (odds > 0 ? odds / 100 : 100 / Math.abs(odds)) * 100) /
    100;
...
export const handleGetSports: RequestHandler = (req, res) => {
  ...
  sportsData.forEach((sport) => {
    sport.activeGames = events.filter(
      (event) =>
...
setInterval(() => {
  const liveEvents = Array.from(sportEvents.values()).filter(
    (event) => event.isLive,
  );
  ...
  liveEvents.forEach((event) => {
    ...
    if (Math.random() < 0.1) {
      ...
      if (isHomeScore) {
        event.homeScore =
          (event.homeScore || 0) +
          (event.sport === "NBA" ? Math.floor(Math.random() * 3) + 1 : 3);
      } else {
        event.awayScore =
          (event.awayScore || 0) +
          (event.sport === "NBA" ? Math.floor(Math.random() * 3) + 1 : 3);
      ...
      const update: LiveUpdate = {
        ...
        lastPlay: isHomeScore
          ? `${event.homeTeam.shortName} scores!`
          : `${event.awayTeam.shortName} scores!`,
        timestamp: new Date(),
      };
