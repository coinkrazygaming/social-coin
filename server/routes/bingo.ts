...
const generateBingoCard = (
  ...
  const bingoCard: BingoCard = {
    ...
    playerName,
    numbers: card,
    markedPositions: card.map((row) =>
      row.map(
        (_, colIndex) => row === 2 && colIndex === 2, // Free space is always marked
      ),
    ),
    isWinner: false,
...
export const handleGetRoomGames: RequestHandler = (req, res) => {
  ...
  const games = Array.from(bingoGames.values()).filter(
    (game) =>
...
export const handleJoinGame: RequestHandler = (req, res) => {
  ...
  if (existingCards.some((card) => card.playerId === userId)) {
    return res
      .status(400)
      .json({ error: "You already have a card for this game" });
  }
...
export const handleGetPlayerCards: RequestHandler = (req, res) => {
  ...
  const cards =
    bingoCards.get(gameId)?.filter((card) => card.playerId === userId) || [];
  res.json(cards);
};
  ...
  for (const [gameId, game] of bingoGames.entries()) {
    if (game.status === "waiting" || game.status === "starting") {
      ...
      game.timeRemaining = Math.max(
        0,
        Math.floor((game.startTime.getTime() - now.getTime()) / 1000),
      );
        ...
        if (numbers) {
          ...
          if (uncalledNumbers.length > 0) {
            const randomIndex = Math.floor(
              Math.random() * uncalledNumbers.length,
            );
