...
export const handleUpdateUserWallet: RequestHandler = (req, res) => {
  try {
    const { userId } = req.params;
    const { goldCoins, sweepsCoins, vipPoints, reason, gameId, gameName } =
      req.body;
    ...
    const newGoldCoins =
      goldCoins !== undefined
        ? currentWallet.goldCoins + goldCoins
        : currentWallet.goldCoins;
    const newSweepsCoins =
      sweepsCoins !== undefined
        ? currentWallet.sweepsCoins + sweepsCoins
        : currentWallet.sweepsCoins;
    const newVipPoints =
      vipPoints !== undefined
        ? currentWallet.vipPoints + vipPoints
        : currentWallet.vipPoints;
    ...
    const updateResult = realTimeDB.updateUserWallet(
      userId,
      newGoldCoins,
      newSweepsCoins,
      newVipPoints,
...
export const handleProcessGameResult: RequestHandler = (req, res) => {
  try {
    ...
    const newSweepsCoins = parseFloat(
      (currentWallet.sweepsCoins + sweepsCoinsChange).toFixed(2),
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to process game result" });
  }
};
...
export const handleGetWalletTransactions: RequestHandler = (req, res) => {
  try {
    ...
    const allTransactions = realTimeDB.getAllTransactions();
    let userTransactions = Array.from(allTransactions.values()).filter(
      (tx) => tx.userId === userId,
    );
...
export const handleGetWalletStats: RequestHandler = (req, res) => {
  try {
    ...
    const wallet = realTimeDB.getUserWallet(userId);
    const allTransactions = realTimeDB.getAllTransactions();
    const userTransactions = Array.from(allTransactions.values()).filter(
      (tx) => tx.userId === userId,
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to fetch wallet statistics" });
  }
};
...
function validateWalletUpdate(
  ...
  if (
    goldCoins !== undefined &&
    (typeof goldCoins !== "number" || !isFinite(goldCoins))
  ) {
  ...
  if (
    sweepsCoins !== undefined &&
    (typeof sweepsCoins !== "number" || !isFinite(sweepsCoins))
  ) {
  ...
  if (
    vipPoints !== undefined &&
    (typeof vipPoints !== "number" || !isFinite(vipPoints))
  ) {
...
function generateTransactionDescription(
  ...
  if (sweepsCoins)
    changes.push(`${sweepsCoins > 0 ? "+" : ""}${sweepsCoins} SC`);
...
function calculateUserStats(userId: string, transactions: any[]): any {
  ...
  return {
    ...
    averageBet:
      totalTransactions > 0
        ? parseFloat((totalWagered / totalTransactions).toFixed(2))
        : 0,
    winRate:
      totalWagered > 0
        ? parseFloat(((totalWon / totalWagered) * 100).toFixed(1))
        : 0,
  };
}
...
function calculateGamesPlayed(transactions: any[]): number {
  return new Set(transactions.filter((tx) => tx.gameId).map((tx) => tx.gameId))
    .size;
}
...
function calculateWalletAchievements(wallet: any, transactions: any[]): any[] {
  ...
  if (wallet.goldCoins > 100000) {
    achievements.push({
      name: "Gold Collector",
      description: "Accumulated over 100,000 Gold Coins",
    });
  }
  ...
  if (wallet.sweepsCoins > 500) {
    achievements.push({
      name: "Sweeps Master",
      description: "Accumulated over 500 Sweeps Coins",
    });
  }
  ...
  if (transactions.length > 1000) {
    achievements.push({
      name: "Active Player",
      description: "Completed over 1,000 transactions",
    });
  }

  const bigWins = transactions.filter(
    (tx) => (tx.winAmount || 0) > 1000,
  ).length;
  if (bigWins > 10) {
...
function calculateTransactionSummary(transactions: any[]): any {
  ...
  const deposits = transactions.filter(
    (tx) => tx.type === "purchase" || tx.type === "deposit",
  ).length;
  const withdrawals = transactions.filter(
    (tx) => tx.type === "withdrawal" || tx.type === "redemption",
  ).length;
  const gameResults = transactions.filter(
    (tx) => tx.type === "game_result",
  ).length;
  ...
  return {
    ...
    amounts: {
      ...
      totalWon: parseFloat(totalWon.toFixed(2)),
      winLossRatio:
        totalWagered > 0 ? parseFloat((totalWon / totalWagered).toFixed(3)) : 0,
    },
  };
}
