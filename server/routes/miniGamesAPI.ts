...
function analyzeGameSession(
  ...
  if (gameHistory.length > 0) {
    const avgScore =
      gameHistory.reduce((sum, h) => sum + h.score / h.maxScore, 0) /
      gameHistory.length;
    ...
    const variance =
      gameHistory.reduce(
        (sum, h) => Math.pow(h.score / h.maxScore - avgScore, 2),
        0,
      ) / gameHistory.length;
  ...
  const recentSessions = userHistory.filter(
    (h) => new Date().getTime() - h.playedAt.getTime() < 24 * 60 * 60 * 1000,
  );
...
function createSecurityEvent(
  ...
  return {
    ...
    gameId: session.gameId,
    actionTaken:
      severity === "critical" ? "session_flagged" : "logged_for_review",
...
export const handleCheckGameCooldown: RequestHandler = (req, res) => {
  try {
    ...
    const timeRemaining = canPlay
      ? 0
      : cooldown.nextAvailable.getTime() - now.getTime();
...
export const handleSubmitGameSession: RequestHandler = (req, res) => {
  try {
    ...
    const userHistory = Array.from(gameSessions.values()).filter(
    ...
    userStats.averageAccuracy =
      (userStats.averageAccuracy * (userStats.totalGames - 1) +
        scorePercentage) /
      userStats.totalGames;
...
export const handleGetUserCooldowns: RequestHandler = (req, res) => {
  try {
    ...
    const userCooldowns = Array.from(cooldowns.values()).filter(
      (c) => c.userId === userId,
    );
...
export const handleGetSecurityEvents: RequestHandler = (req, res) => {
  try {
    ...
    let events = Array.from(securityEvents.values()).sort(
...
export const handleGetGameAnalytics: RequestHandler = (req, res) => {
  try {
    ...
    let sessions = Array.from(gameSessions.values()).filter(
      (s) => s.playedAt >= startDate,
    );
