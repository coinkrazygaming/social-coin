...
export function MiniGamesPlatform() {
  ...
  const [cooldowns, setCooldowns] = useState<Map<string, CooldownData>>(
    new Map(),
  );
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      const cooldownResponse = await fetch(
        `/api/mini-games/${user.id}/cooldowns`,
      );
  ...
  const checkCooldown = (
    gameId: string,
  ): { canPlay: boolean; timeRemaining: number } => {
  ...
  const analyzeSecurityData = (session: GameSession): SecurityEvent[] => {
    ...
    const avgScore =
      userHistory.length > 0
        ? userHistory.reduce((sum, h) => sum + h.score / h.maxScore, 0) /
          userHistory.length
        : 0;
  ...
  const logGameSession = async (session: GameSession) => {
    try {
      ...
      const aiLogResponse = await fetch(
        "/api/ai-employees/mini-games/log-session",
        {
          method: "POST",
      ...
      const cooldown: CooldownData = {
        ...
        nextAvailable: new Date(
          session.playedAt.getTime() + 24 * 60 * 60 * 1000,
        ),
        attemptsToday: 1,
      };
          ...
          <div className="mb-4 flex justify-between items-center">
            ...
            <div className="text-white text-center">
              ...
              <p className="text-purple-200">
                Max Reward: {selectedGame.maxReward} SC
              </p>
            </div>
          </div>
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredGames.map((game) => {
            const { canPlay, timeRemaining } = user
              ? checkCooldown(game.id)
              : { canPlay: false, timeRemaining: 0 };
              ...
              <Card className="bg-gray-800 border-gray-700">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-2xl font-bold text-green-400">
                    {gameHistory
                      .reduce((sum, game) => sum + game.scEarned, 0)
                      .toFixed(2)}
                  </div>
