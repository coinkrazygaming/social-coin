...
export function AIEmployeeDashboard() {
      ...
      if (statusResponse.ok) {
        ...
        setEmployees((prev) =>
          prev.map((emp) => ({
            ...
            ...statusData[emp.id],
            lastActivity: new Date(
              statusData[emp.id]?.lastActivity || emp.lastActivity,
            ),
          })),
        );
      }
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          const aiResponse: AIMessage = {
            ...
            fromId: employeeId,
            fromName:
              employees.find((e) => e.id === employeeId)?.name || "AI Employee",
            toId: user?.id || "admin",
    ...
    switch (employee.department) {
      ...
      case "mini_games":
        if (
          ...
          userMessage.toLowerCase().includes("cheat")
        ) {
  ...
  const handleAlertAction = async (
    alertId: string,
    action: "approve" | "deny" | "investigate",
    notes?: string,
  ) => {
    try {
      const response = await fetch(
        `/api/ai-employees/alerts/${alertId}/action`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        <Card className="bg-gray-800 border-gray-700 p-8">
          <div className="text-center">
            ...
            <p className="text-gray-400">
              Admin privileges required to access AI Employee Dashboard
            </p>
          </div>
        </Card>
      </div>
              ...
              <Card className="bg-gray-800 border-gray-700">
                <CardContent className="p-6 text-center">
                  ...
                  <div className="text-2xl font-bold text-purple-400">
                    {Math.round(
                      employees.reduce((sum, e) => sum + e.successRate, 0) /
                        employees.length,
                    )}
                    %
                  </div>
          ...
          <TabsContent value="employees" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {employees.map((employee) => (
                <Card
                  key={employee.id}
                  className="bg-gray-800 border-gray-700 hover:border-gold/50 transition-all"
                >
                  <CardHeader>
                ...
                <Card key={alert.id} className="bg-gray-800 border-gray-700">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-4 h-4 rounded-full ${getSeverityColor(alert.severity)}`}
                        />
                        <div>
                          <CardTitle className="text-white text-lg">
                            {alert.title}
                          ...
                          <p className="text-gray-400 text-sm">
                            From:{" "}
                            {
                              employees.find((e) => e.id === alert.employeeId)
                                ?.name
                            }{" "}
                  ...
                  <div className="space-y-2">
                    {employees.map((employee) => (
                      ...
                      >
                        ...
                        {employee.name}
                        <div
                          className={`ml-auto w-2 h-2 rounded-full ${getStatusColor(employee.status)}`}
                        />
                      </Button>
                    ))}
                  </div>
                        ...
                        <ScrollArea className="flex-1 mb-4">
                          <div className="space-y-4">
                            {messages
                              .filter(
                                (m) =>
                                  (m.fromId === selectedEmployee.id ||
                                    m.toId === selectedEmployee.id) &&
                                  (m.fromId === user?.id ||
                                    m.toId === user?.id),
                              )
                              .map((message) => (
            ...
            <Card className="bg-gray-800 border-gray-700">
              ...
              <CardContent className="space-y-6">
                <div className="text-white">
                  <p className="mb-4">
                    Configure AI employee behavior and permissions
                  </p>
                  <p className="text-sm text-gray-400">
                    Advanced settings for AI employee management will be
                    available here. This includes performance thresholds, alert
                    sensitivity, and department-specific configurations.
                  </p>
                </div>
              </CardContent>
            </Card>
