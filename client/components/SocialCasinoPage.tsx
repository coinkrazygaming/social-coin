...
export default function SocialCasinoPage() {
  ...
  const [selectedGame, setSelectedGame] = useState<SocialCasinoGame | null>(null);
  ...
  const loadGamesAndStats = async () => {
    try {
      ...
      const coinKrazyGames: SocialCasinoGame[] = DEFAULT_COINKRAZY_SLOTS.map(slot => ({
        id: slot.id,
        name: slot.name,
        provider: 'CoinKrazy',
  ...
  const filterGames = () => {
    ...
    if (selectedCategory !== 'all') {
      if (selectedCategory === 'featured') {
        ...
        filtered = filtered.sort((a, b) => b.total_spins_today - a.total_spins_today).slice(0, 20);
      } else {
            ...
            <div className="flex items-center justify-center mb-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-lg">
                  {realTimeStats?.total_games_available || 0}+ Premium social casino games with real-time jackpots and massive wins!
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 max-w-4xl mx-auto">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-sweep flex items-center justify-center">
                    ...
                    {realTimeStats.sc_earned_today.toLocaleString(undefined, { maximumFractionDigits: 2 })}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-casino-green flex items-center justify-center">
                    ...
                    {Math.max(realTimeStats.max_win_available_sc, realTimeStats.max_win_available_gc).toLocaleString()}
                  </div>
            ...
            {realTimeStats?.biggest_win_today && (
              <div className="mt-6 p-4 bg-gradient-to-r from-gold/20 to-yellow-400/20 rounded-lg border border-gold/30 max-w-md mx-auto">
                <div className="flex items-center justify-center mb-2">
                  ...
                  <span className="font-semibold text-gold">Biggest Win Today</span>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">
                    {realTimeStats.biggest_win_today.amount.toLocaleString()} {realTimeStats.biggest_win_today.currency}
                  </div>
              ...
              <Card key={game.id} className="group hover:shadow-lg transition-shadow overflow-hidden">
                <div className="relative">
                  ...
                  <div className="absolute top-2 right-2">
                    <Badge className={
                      game.game_type === 'BOTH' ? 'bg-gold text-black' :
                      game.game_type === 'GC' ? 'bg-blue-500' : 'bg-sweep'
                    }>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="text-center p-1 bg-muted/50 rounded">
                      <div className="font-semibold">{game.total_spins_today}</div>
                      <div className="text-muted-foreground">Spins Today</div>
                    </div>
                    <div className="text-center p-1 bg-muted/50 rounded">
                      <div className="font-semibold text-gold">{game.biggest_win_today.toLocaleString()}</div>
        ...
        <Dialog open={showGameModal} onOpenChange={setShowGameModal}>
          <DialogContent className="max-w-md">
            ...
            <GameModeSelector
              ...
              supportedModes={selectedGame.game_type === 'BOTH' ? ['GC', 'SC'] : 
                            selectedGame.game_type === 'GC' ? ['GC'] : ['SC']}
