...
export const AdminAnalyticsDashboard: React.FC<AdminAnalyticsDashboardProps> = ({ className = '' }) => {
      ...
      recommendations: [
        {
          ...
          description: 'Golden Treasure Hunt showing negative revenue growth (-2.3%). Analysis suggests RTP is too high for current player engagement levels.',
          actionPlan: [
        ...
        },
        {
          ...
          description: 'Diamond Fortune showing exceptional performance (22.1% growth). Capitalize on this momentum with targeted marketing.',
          actionPlan: [
        ...
        },
        {
          ...
          description: 'Average session time (23.5 minutes) is below industry standard (28 minutes). Implement engagement boosters.',
          actionPlan: [
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                {analyticsData.miniGames.map((game, index) => (
                  ...
                  >
                    <Card className={`border-2 ${game.isVerified ? 'border-green-500/30' : 'border-yellow-500/30'}`}>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={`text-xs ${
                              game.revenueGrowth >= 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                            }`}>
                              {game.revenueGrowth >= 0 ? '+' : ''}{formatPercentage(game.revenueGrowth)}
                            </Badge>
                          </div>
                        </div>
                        ...
                        <div className="space-y-3">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <span className="text-green-400 ml-2">{formatCurrency(game.revenue)}</span>
                            </div>
                            <div>
                              <span className="text-gray-400">Players:</span>
                              <span className="text-blue-400 ml-2">{formatNumber(game.totalPlayers)}</span>
                            </div>
                            <div>
                              <span className="text-gray-400">RTP:</span>
                              <span className="text-yellow-400 ml-2">{formatPercentage(game.rtp)}</span>
                            </div>
                            <div>
                              <span className="text-gray-400">Popularity:</span>
                              <span className="text-purple-400 ml-2">{game.popularity}/100</span>
                            </div>
                          </div>
                          ...
                          <div className="text-xs text-gray-400">
                            ...
                            <p>Last check: {game.lastVerification.toLocaleTimeString()}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <ul className="space-y-1">
                              {rec.actionPlan.map((action, actionIndex) => (
                                <li key={actionIndex} className="text-sm text-gray-300 flex items-start">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <span className="text-green-400 ml-2">{rec.estimatedImpact}</span>
                            </div>
                            <div>
                              <span className="text-gray-400">Implementation:</span>
                              <span className="text-blue-400 ml-2">{rec.implementationTime}</span>
                            </div>
                          </div>
                        </div>
                      </CardContent>
                  ...
                  <div className="space-y-2">
                    {analyticsData.timeSeriesData.map((data, index) => (
                      <div key={index} className="flex items-center justify-between p-2 bg-card/50 rounded">
                        ...
                        <span className="text-sm text-green-400">{formatCurrency(data.revenue)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="text-white">{formatPercentage(analyticsData.overview.conversionRate)}</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="text-white">{formatPercentage(analyticsData.overview.houseEdge)}</span>
                      </div>
                      <Progress value={analyticsData.overview.houseEdge} className="h-2" />
                    </div>
