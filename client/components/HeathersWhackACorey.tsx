...
export function HeathersWhackACorey({
  userId,
  username,
  onGameComplete,
}: HeathersWhackACoreyProps) {
  ...
  const [hitEffects, setHitEffects] = useState<
    { id: number; x: number; y: number; type: string; timestamp: number }[]
  >([]);
  ...
  const spawnMole = useCallback(() => {
    ...
    const availableHoles = holes.filter(
      (hole) => !activeMoleHoles.has(`${hole.x}-${hole.y}`),
    );
        ...
        prevMoles.map((mole) => {
          if (mole.active && mole.animationState === "visible") {
            const distance = Math.sqrt(
              (clickX - mole.x) ** 2 + (clickY - mole.y) ** 2,
            );
  ...
  const updateMoles = useCallback(() => {
      ...
      prevMoles
        .map((mole) => {
          ...
          if (mole.animationState === "rising" && timeAlive > 300) {
            ...
            mole.animationState === "visible" &&
            timeAlive > mole.visibleTime
          ) {
    ...
    setHitEffects((prev) =>
      prev.filter((effect) => currentTime - effect.timestamp < 1000),
    );
  }, []);
