...
export const BonusManagement: React.FC = () => {
  ...
  const loadBonusData = async () => {
    try {
      ...
      const realBonuses: Bonus[] = [
        {
          ...
          type: "welcome",
          description:
            "New player welcome package with 100% match bonus plus 50 free spins",
          value: 100,
          valueType: "percentage",
          currency: "GC",
          minDeposit: 10,
        ...
        },
        {
          ...
          type: "referral",
          description:
            "Get 100,000 GC for each friend you refer who makes a deposit",
          value: 100000,
          valueType: "fixed_amount",
          currency: "GC",
          wageringRequirement: 1,
      ...
      const realStats: BonusStats = {
        ...
        totalClaimed: realBonuses.reduce(
          (sum, b) => sum + b.analytics.claimed,
          0,
        ),
        totalValue: realBonuses.reduce(
          (sum, b) => sum + b.analytics.totalValue,
          0,
        ),
        conversionRate:
          realBonuses.reduce((sum, b) => sum + b.analytics.conversionRate, 0) /
          realBonuses.length,
        popularBonus:
          realBonuses.sort(
            (a, b) => b.analytics.claimed - a.analytics.claimed,
          )[0]?.name || "None",
        recentActivity: 1247,
      };
  ...
  const updateBonus = (bonusId: string, updates: Partial<Bonus>) => {
    setBonuses((prev) =>
      prev.map((bonus) =>
        bonus.id === bonusId
          ? { ...bonus, ...updates, updatedAt: new Date() }
          : bonus,
      ),
    );
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <AnimatePresence>
              {filteredBonuses.map((bonus) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className={`bg-gray-800 border-gray-700 ${bonus.isActive ? "ring-1 ring-green-500/20" : "opacity-75"}`}
                    >
                      <CardHeader>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <Badge
                                  variant={
                                    bonus.isActive ? "default" : "secondary"
                                  }
                                >
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="text-white">
                              {bonus.usageCount}{" "}
                              {bonus.usageLimit ? `/ ${bonus.usageLimit}` : ""}
                            </span>
                          </div>
                          {bonus.usageLimit && (
                            <Progress
                              value={
                                (bonus.usageCount / bonus.usageLimit) * 100
                              }
                              className="h-2"
                            />
                          )}
                        ...
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div className="text-center">
                            <div className="text-green-400 font-semibold">
                              {bonus.analytics.claimed}
                            </div>
                            <div className="text-gray-400">Claimed</div>
                          </div>
                          <div className="text-center">
                            <div className="text-blue-400 font-semibold">
                              {bonus.analytics.completed}
                            </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      value={newBonus.type}
                      onValueChange={(value) =>
                        setNewBonus((prev) => ({ ...prev, type: value as any }))
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setNewBonus((prev) => ({
                            ...prev,
                            wageringRequirement: parseFloat(e.target.value),
                          }))
                        }
                      />
                    </div>
                      ...
                      >
                        <div>
                          ...
                          <div className="text-gray-400 text-sm">
                            {bonus.analytics.claimed} claimed
                          </div>
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-20 bg-gray-700 rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${(count / bonuses.length) * 100}%`,
                            }}
                          />
                        </div>
      ...
      <Dialog
        open={!!selectedBonus}
        onOpenChange={() => setSelectedBonus(null)}
      >
          ...
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {selectedBonus && (
                <>
                  <div
                    className={`p-2 rounded ${getBonusTypeColor(selectedBonus.type)}`}
                  >
                    {React.createElement(getBonusTypeIcon(selectedBonus.type), {
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white capitalize">
                        {selectedBonus.type.replace("_", " ")}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Value:</span>
                      <span className="text-white">
                        {formatValue(selectedBonus)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Currency:</span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">
                        {selectedBonus.maxBonus
                          ? selectedBonus.maxBonus.toLocaleString()
                          : "Unlimited"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Wagering:</span>
                      <span className="text-white">
                        {selectedBonus.wageringRequirement}x
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Valid For:</span>
                      <span className="text-white">
                        {selectedBonus.validDays} days
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">
                        {selectedBonus.analytics.claimed.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Completed:</span>
                      <span className="text-white">
                        {selectedBonus.analytics.completed.toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Conversion Rate:</span>
                      <span className="text-white">
                        {selectedBonus.analytics.conversionRate.toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Total Value:</span>
                      <span className="text-white">
                        {selectedBonus.analytics.totalValue.toLocaleString()}{" "}
                        {selectedBonus.currency}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="text-white">
                        {selectedBonus.analytics.averageCompletion} days
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-white capitalize">
                      {selectedBonus.eligibleUsers}
                    </div>
                  </div>
                  <div>
                    <span className="text-gray-400">Auto Apply:</span>
                    <div className="text-white">
                      {selectedBonus.triggers.autoApply ? "Yes" : "No"}
                    </div>
                  </div>
                  <div>
                    <span className="text-gray-400">Requires Code:</span>
                    <div className="text-white">
                      {selectedBonus.triggers.requiresCode
                        ? `Yes (${selectedBonus.triggers.bonusCode})`
                        : "No"}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="text-white capitalize">
                      {selectedBonus.triggers.triggerEvent || "Manual"}
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditingBonus((prev) =>
                        prev
                          ? { ...prev, validDays: parseInt(e.target.value) }
                          : null,
                      )
                    }
                  />
