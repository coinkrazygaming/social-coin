...
export function FlickenMyBean({ userId, username, onGameComplete }: FlickenMyBeanProps) {
  ...
  const flickBean = useCallback((startX: number, startY: number, endX: number, endY: number) => {
  ...
  const updateBeans = useCallback(() => {
    setBeans(prevBeans => prevBeans.map(bean => {
      ...
      if (newX <= bean.size || newX >= CANVAS_WIDTH - bean.size) {
        ...
        newX = Math.max(bean.size, Math.min(CANVAS_WIDTH - bean.size, newX));
      }
  ...
  const draw = useCallback(() => {
    ...
    beans.forEach(bean => {
      ...
      ctx.beginPath();
      ctx.arc(bean.x - bean.size * 0.3, bean.y - bean.size * 0.3, bean.size * 0.3, 0, Math.PI * 2);
      ctx.fill();
    });
    ...
    if (isDragging) {
      ...
      if (distance > 20) {
        ...
        ctx.fillStyle = powerPercent > 80 ? '#FF0000' : powerPercent > 50 ? '#FFD700' : '#00FF00';
        ctx.fillRect(BEAN_START_X - 30, BEAN_START_Y - 60, 60, 10);
        
        ctx.fillStyle = '#FFFFFF';
        ctx.fillRect(BEAN_START_X - 30, BEAN_START_Y - 60, (60 * powerPercent) / 100, 10);
  ...
  const accuracy = beansFlicked > 0 ? Math.round((targetsHit / beansFlicked) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-blue-400">{beansFlicked}</div>
            ...
            {isPlaying && (
              <div className="text-center space-y-2">
                <div className="text-lg font-semibold">
                  {isDragging ? 'Release to flick!' : 'Click and drag from START to flick beans at targets!'}
                </div>
