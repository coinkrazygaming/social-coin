...
import { Send, Users, Mic, MicOff, Settings, Download, Trash2, Bot, User, Shield } from 'lucide-react';
...
export const AdminChatWindow: React.FC<AdminChatWindowProps> = ({ onUnreadChange }) => {
    ...
    <div className="flex flex-col h-full max-h-[600px]">
      ...
      <div className="p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            AI Employees ({employees.filter(e => e.status === 'online').length} online)
          </h4>
          ...
          {employees.map((employee) => (
            ...
            >
              <div className="relative">
                ...
                <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border-2 border-white dark:border-gray-700 ${getEmployeeStatus(employee)}`} />
              </div>
              <div>
              ...
              >
                {message.senderType !== 'admin' && (
                  <Avatar className="w-8 h-8 mt-1">
                    <AvatarFallback className={`text-white text-xs ${getMessageTypeColor(message)}`}>
                      {message.senderType === 'ai_employee' ? (
                        employees.find(e => e.id === message.senderId)?.avatar || '🤖'
                      ) : (
                ...
                <div className={`max-w-[80%] ${message.senderType === 'admin' ? 'order-first' : ''}`}>
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
        ...
        <div className="flex items-center justify-between mt-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>{employees.filter(e => e.status === 'online').length} AI agents online</span>
        </div>
      </div>
      ...
      <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <div className="text-xs text-blue-700 dark:text-blue-300">
          <strong>LuckyAI</strong> coordinates all operations and provides oversight. 
          Ask about status, reports, or any casino operations.
        </div>
      </div>
    </div>
  );
