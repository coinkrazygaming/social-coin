...
import { MiniGamePlay, User as UserType, Transaction, PayPalPayment } from '@shared/types';
...
export default function AdminPanel() {
  ...
  const colinShotsHistory = miniGameHistory.filter(play => play.gameType === 'colin-shots');
  const totalSCPaidOut = miniGameHistory.reduce((sum, play) => sum + play.scEarned, 0);
  ...
  const stats = {
    ...
    totalRevenue,
    avgScoreColinShots: colinShotsHistory.length > 0 
      ? Math.round((colinShotsHistory.reduce((sum, play) => sum + play.score, 0) / colinShotsHistory.length) * 100) / 100 
      : 0
  };
    ...
    <div className="min-h-screen p-4">
      <div className="container mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">CoinKrazy Casino Management</p>
            </div>
          </div>
          ...
          <Alert>
            ...
            <AlertDescription>
              Welcome, {user.username}! You have full administrative access to CoinKrazy.
            </AlertDescription>
          </Alert>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Mini Game Plays</CardTitle>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-sweep">{stats.totalSCPaidOut}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-casino-green">${stats.totalRevenue.toFixed(2)}</div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Avg Colin Shots</CardTitle>
              <Trophy className="h-4 w-4 text-gold" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-gold">{stats.avgScoreColinShots}/25</div>
            </CardContent>
          </Card>
        </div>
                  ...
                  {colinShotsHistory.length === 0 ? (
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {colinShotsHistory.map((play) => {
                        ...
                        return (
                          <div key={play.id} className="flex items-center justify-between p-4 bg-muted/20 rounded-lg">
                            <div className="flex items-center space-x-4">
                              <div className="w-10 h-10 bg-gradient-to-br from-gold to-yellow-400 rounded-full flex items-center justify-center">
                                🏀
                              </div>
                              <div>
                                <div className="font-medium">{user?.username || 'Unknown Player'}</div>
                            ...
                            <div className="flex items-center space-x-4">
                              <div className="text-center">
                                <div className={`font-bold ${getScoreColor(play.score, play.maxScore)}`}>
                    ...
                    <div key={user.id} className="flex items-center justify-between p-4 bg-muted/20 rounded-lg">
                      <div className="flex items-center space-x-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                          user.role === 'admin' ? 'bg-gradient-to-br from-casino-red to-red-600' :
                        ...
                        <div>
                          <div className="font-medium flex items-center space-x-2">
                            ...
                            <Badge variant="outline" className={
                              user.role === 'admin' ? 'border-casino-red/30 text-casino-red' :
                              user.role === 'staff' ? 'border-blue-500/30 text-blue-400' :
          ...
          <TabsContent value="payments" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Track all PayPal transactions and Gold Coin purchases</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3 max-h-96 overflow-y-auto">
                        ...
                        <div key={payment.id} className="flex items-center justify-between p-4 bg-muted/20 rounded-lg">
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="text-center">
                              ...
                              <div className="text-xs text-muted-foreground">SC Bonus</div>
                            </div>
