...
export function StaffPanel() {
  ...
  const handleRedemptionReview = async (requestId: string, status: 'staff_approved' | 'denied', notes: string) => {
    try {
      const response = await fetch(`/api/users/redemptions/${requestId}/staff-review`, {
        method: 'POST',
        headers: { 
      ...
      if (response.ok) {
        fetchStaffData();
        alert(`Redemption request ${status.replace('staff_', '')}! ${status === 'staff_approved' ? 'Forwarded to admin for final approval.' : ''}`);
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <Card className="bg-gray-800 border-gray-700">
            <CardContent className="p-4 text-center">
              ...
              <div className="text-2xl font-bold text-purple-500">
                {redemptionRequests.filter(r => r.status === 'staff_review').length}
              </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="redemptions" className="space-y-6">
            <Card className="bg-gray-800 border-gray-700">
              <CardHeader>
                <CardTitle className="text-white">Prize Redemption Requests</CardTitle>
              </CardHeader>
              <CardContent>
                {redemptionRequests.length > 0 ? (
                      ...
                      <div key={request.id} className="p-4 bg-gray-700 rounded-lg">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            ...
                            <div className="mt-2">
                              <Badge className={request.kycCompleted ? 'bg-green-600' : 'bg-red-600'}>
                                KYC: {request.kycCompleted ? 'Verified' : 'Pending'}
                              </Badge>
                            </div>
                          </div>
                        ...
                        {request.status === 'pending' ? (
                          <div className="flex gap-2">
                            <Button
                              ...
                              onClick={() => {
                                const notes = prompt('Staff review notes (optional):') || '';
                                handleRedemptionReview(request.id, 'staff_approved', notes);
                              }}
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-400">No redemption requests to review.</p>
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent>
                {refundRequests.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-400">No refund requests to review.</p>
                  </div>
                )}
              </CardContent>
            </Card>
